...
router.post('/', verifyAdmin, async (req, res) => {
  try {
    const { question, answer, category, display_order = 0, is_active = 1 } = req.body;
    ...
    const [result] = await db.execute(
      `INSERT INTO faqs (question, answer, category, display_order, is_active)
       VALUES (?, ?, ?, ?, ?)`,
      [question, answer, category.trim(), display_order, is_active ? 1 : 0]
    );
...
router.put('/:id', verifyAdmin, async (req, res) => {
  try {
    ...
    await db.execute(
      `UPDATE faqs 
       SET question = ?, answer = ?, category = ?, display_order = ?, is_active = ?, updated_at = NOW()
       WHERE id = ?`,
      [
        question,
        answer,
