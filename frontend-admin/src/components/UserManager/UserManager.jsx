...
export default function UserManager() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      const url = isEditing ? `${API_USER_URL}/${form.id}` : API_USER_URL;
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await fetch(`${API_USER_URL}/${id}`, { method: "DELETE" });
    ...
    <div className="solution-manager-container" style={{ display: "flex", gap: "24px", marginTop: "20px", flexDirection: "row-reverse" }}>
      ...
      <div className="panel" style={{ flex: 1.8 }}>
        <div className="panel-header" style={{ justifyContent: "space-between" }}>
          ...
          <button className="btn btn-success btn-sm" onClick={fetchUsers}><RefreshCw size={14}/> Làm mới</button>
        </div>
