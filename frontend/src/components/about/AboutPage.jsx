                            ...
                            <div className="legal-cards-grid">
                                {loading ? (
                                    ...
                                    legalDocuments.map((doc) => (
                                        <div key={doc.id} className="legal-card">
                                            <h4 className="legal-card-title">{doc.title}</h4>
                                            {doc.description && (
                                                <p className="legal-card-desc">{doc.description}</p>
                                            )}
                                            {doc.issue_date && (
                                                <p className="legal-card-date">Ngày {new Date(doc.issue_date).toLocaleDateString('vi-VN')}</p>
                                            )}
                                            {doc.file_url && (
                                                <a href={doc.file_url} target="_blank" rel="noopener noreferrer" className="legal-card-link doc-link">Xem chi tiết →</a>
                                            )}
                                        </div>
                                    ))
                                ) : (
                            ...
                            <div className="legal-cards-grid">
                                {loading ? (
                                    ...
                                    authorities.map((auth) => (
                                        <div key={auth.id} className="legal-card">
                                            <h4 className="legal-card-title">{auth.name}</h4>
                                            {auth.description && (
                                                <p className="legal-card-desc">{auth.description}</p>
                                            )}
                                        </div>
                                    ))
                                ) : (
